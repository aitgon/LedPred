...
modelperf.obj <- ModelPerformance$new(x = x, y = y, valid.times=valid.times)
...
cv.probs.labels <- evaluateModelPerformance(x = x, y = y, valid.times=valid.times)
testthat::expect_equal(as.numeric(cv.probs.labels$probs[[5]][1]), 0.2338657, tolerance=1e-6)
}
)
