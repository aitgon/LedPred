...
test_that("Tune cost gamma (crm.features.tab)", {
...
test_that("Tune cost gamma (crm.features.granges)", {

	# Get crm_matrix
	data(crm.features)

c.g.obj <- mcTune(data.granges = crm.features, cl = 1, ranges = list(gamma = gamma_vector, cost = cost_vector), kernel=kernel, scale=scale, valid.times=valid.times)
